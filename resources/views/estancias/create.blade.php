            ...
            @foreach($carreras as $nombreCarrera => $icono)
                <div x-data="{ open: false }" class="bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
                    <button @click="open = !open" class="w-full flex items-center justify-between p-5 hover:bg-gray-50 transition text-left">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                @php
                                    $conteo = $estancias->filter(fn($e) => optional($e->alumno)->carrera === $nombreCarrera)->count();
                                @endphp
                                <span class="text-sm text-blue-600 font-medium">{{ $conteo }} Alumnos activos</span>
                            </div>
                        </div>
                    ...
                    <div x-show="open" x-cloak x-collapse class="bg-gray-50 border-t border-gray-100 p-5">
                        <div class="space-y-3">
                            @forelse($estancias->filter(fn($e) => optional($e->alumno)->carrera === $nombreCarrera) as $estancia)
                                <div class="flex items-center justify-between bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                                    ...
                                    <div class="text-right">
                                        ...
                                        <p class="text-sm font-mono font-bold text-gray-700">{{ \Carbon\Carbon::parse($estancia->fecha_fin)->format('d/m/Y') }}</p>
                                    </div>
                                </div>
                            @empty
                                <div class="text-center py-6">
                                    <p class="text-gray-400 text-sm italic">No hay alumnos en proceso para esta carrera.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
