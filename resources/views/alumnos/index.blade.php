    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white shadow-xl rounded-2xl p-6 border border-gray-100 sticky top-8">
                ...
                <form action="{{ route('alumnos.store') }}" method="POST" class="space-y-4" id="alumnoForm">
                    @csrf
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select name="cuatrimestre" id="selectCuatrimestre"
                                class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 outline-none cursor-pointer transition-all"
                                required>
                        ...
                        <div>
                            ...
                            <select name="grupo" id="selectGrupo"
                                class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 outline-none cursor-pointer transition-all"
                                required>
                                <option value="" disabled selected>Primero selecciona cuatrimestre</option>
                            </select>
                        </div>
                    </div>
...
@push('scripts')
<script>
    // Actualizar grupo según cuatrimestre seleccionado
    document.getElementById('selectCuatrimestre').addEventListener('change', function () {
        const cuatrimestre = this.value;
        const grupoSelect  = document.getElementById('selectGrupo');
        grupoSelect.innerHTML = '<option value="" disabled selected>Selecciona grupo</option>';

        ['A', 'B', 'C', 'D'].forEach(letra => {
            const opt       = document.createElement('option');
            opt.value       = cuatrimestre + ' ' + letra;
            opt.textContent = cuatrimestre + ' ' + letra;
